...
- `showDirectNames`: 是否在地图上该点下方直接常驻显示地标名称（布尔值，默认 `false`。适合点位数量较少的主题地图，如三山五岳、名门帮会等）。
- `points`: 点位数组。每个点支持以下字段：
  - `id`: 点位唯一英文/拼音标识（必填）
  - `name`: 点位或区域名称（必填）
  - `lat`, `lon`: 中心点经纬度（必填，用于放置地标气泡、计算地图外接大盘视野）
  - `category`: 分类标签（可选）
  - `badgeColor`: 独立气泡/区域填充微标色（可选，如 `#1ABC9C`）
  - `description`: 介绍说明（可选）
  - `boundaryType`: 区域几何类型（可选，支持 `"polygon"` 闭合多边形）
  - `boundary`: 多边形顶点经纬度集合（可选，格式为 `[[lon1, lat1], [lon2, lat2], ...]`，遵循国际 GeoJSON 规范经度在前、纬度在后）

### 打卡判定算法机制
- **多边形区域模式**：若点位配置了 `boundaryType: "polygon"` 且包含有效多边形坐标（$\ge 3$ 个顶点），客户端将采用**射线交叉法（Ray-Casting Point-in-Polygon Algorithm）**进行判断。用户的任意历史足迹落在该闭合多边形区域内，即刻判定为已探索打卡！地图上也会同时绘制半透明区域覆盖轮廓。
- **中心点物理距离模式**：若未配置多边形，或足迹不在多边形内，客户端自动回退使用中心点球面物理距离判定（$\le 1000$ 米视为探索打卡）。
